...
const ProductDetails = () => {
  ...
  const { data: product, error, isLoading } = getProductBySlug(slug);

  console.log("product", product);

  const { items, addItem, removeItem, incrementItem, decrementItem } =
    useCartStore();
  ...
  const increaseQuantity = () => {
    if (quantity >= product.maxQuantity) {
      toast.show("You can't add more than " + product.maxQuantity, {
        type: "warning",
        placement: "bottom",
        duration: 3000,
      });
